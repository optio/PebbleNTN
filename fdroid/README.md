...
Fully automated. `AutoUpdateMode: Version v%v` makes F-Droid detect each new `vX.Y.Z`
tag on its own, and the release workflow (`.github/workflows/release.yml`) writes the
matching `fastlane/metadata/android/en-US/changelogs/<versionCode>.txt` into the tagged
commit as part of every release — so each F-Droid build ships its own release notes with
no manual step. The changelog body is generated by `scripts/changelog.py --fdroid` (the
version's user-facing commits, capped at F-Droid's 500-character limit); versionCode =
`major*10000 + minor*100 + patch` (e.g. `0.0.19` → `19`).

...
`Fußgängerübergang nehmen` and `Nach Osten/Süden/…` compass headings; `DistanceParser` never parsing
Google Maps distances (Maps separates value and unit with U+00A0, which Java's `\s` excludes); and
the locale gate reading `Locale.getDefault()` rather than the notification's language.
...
notifications matched nothing. Diagnosis: the bundled rules were English-only, and the redactor's
keep-list was English-only too, so the maneuver words were stripped and the log couldn't even be
used to author the fix.

**Localized rulesets.** Added `bundled/google-maps/{it,fr,es,de,nl}.json`, generated by
`tools/rule-workbench/gen_localized_google_maps.py` from a compact per-language keyword table. They
mirror en.json's priority ladder (arrive > roundabout > u-turn > sharp > slight > keep > turn >
continue) and extractors; only the condition regexes differ. (ARRIVE was moved to the bottom of that
ladder on 2026-07-29 — see the section at the top of this file.) Turn L/R is the catch-all and matches
the localized "to the side" phrase; sharp/slight/keep are refinements above it (modifier + direction,
order-independent) that degrade to a plain turn if they miss. ARRIVE matches the title only (the ETA
in subText carries a localized "arrive" word). The app already loads every locale `.json` and the
engine filters by device language, so no app change was needed. **Fixtures are synthetic** (authored
from documented Google Maps phrasings) — the wording, especially sharp/slight/keep, needs validation
against real captures per language. 37 localized fixtures added; both regression harnesses
(`workbench.py`, `GoogleMapsRulesRegressionTest`) now load all locale files — 54/54 green.

**Multilingual redaction.** `Redactor.KEEP` gains maneuver/direction words for it/fr/es/de/nl
(destra, tournez, abbiegen, rotonde, …). These carry no personal information; road/place names are
exactly the words NOT on the list, so they still redact. Now a privacy-safe log from a non-English
user preserves the maneuver structure and can drive that language's rules. `RedactorTest` proves the
Italian/German maneuver survives while the road name is redacted.

**Export locale.** The privacy-safe export metadata now includes the device `locale` (e.g. "it-IT"),
which the spec (spec/300-data/ExportFormat.md) already called for but the implementation omitted — it
tells a maintainer which language a shared log can improve, without any personal data.
